    ...
    [;; the run time for sorted is O(nlogn), where n is the 
     ...
     (define (not-same-adjacent-lst num lon)
       (cond
         [(empty? lon) empty]
         [(= (first lon) num) 
          (not-same-adjacent-lst num (rest lon))]
         [(empty? (rest lon)) lon]
         [(not (= (first lon) (second lon)))
          (cons (first lon)
                (not-same-adjacent-lst (first lon) (rest lon)))]
         [else (not-same-adjacent-lst (second lon) (rest (rest lon)))]))
     ;;*****************************************************
     ;; (wrapper alist)
     ;; Step 1. O(1), see below:
     ;;      empty?: O(1)
     ...
     (define (wrapper alist)
       (cond
         ...
         [(not (= (first alist) (second alist)))
          (cons (first alist)
                (not-same-adjacent-lst (first alist) (rest alist)))]
         [else (not-same-adjacent-lst (first alist) (rest alist))]))] 
    (wrapper sorted)))
